<template>
  ...
  >
    ...
    >
      <template v-for="(classification, index) of store.classificationList" :key="'classification-' + classification.id + '-' + index">
        <!-- 分隔线：在非顶部布局下，除第一个分类外，每个分类前添加分隔线 -->
        <li
          v-if="index > 0 && store.setting.classification.layout !== 'top'"
...
function wheelSwitchClassification(e: any) {
  ...
  if (n !== index) {
    ...
    if (
      ...
      // 获取判断可以容纳多少个分类
      let classificationNum = Math.trunc(element.clientHeight / 44);
      // 获取滚动条实例
