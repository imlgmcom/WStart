        ...
        <div id="content" :style="{ maxHeight: height - 34 - 40 + 'px' }">
          <div class="mx-2">
            ...
            <div
              ...
              "
            >
              <span class="block">{{ store.language.backgroundImage }}</span>
              <div class="flex items-center mt-1.5">
                <NInput
                  class="flex-1 mr-2"
                  v-model:value="form.data.backgroundImage"
                  clearable
                  size="small"
                  placeholder=""
                  @blur="checkBackgroundImageUrl"
                ></NInput>
                <NButton
                  type="primary"
                  size="small"
              ...
              <!-- 背景图预览 -->
              <div class="mt-2 p-2 border border-solid rounded" style="border-color: #e0e0e0;">
                <div class="text-sm text-gray-500 mb-1">{{ store.language.preview }}</div>
                <div 
                  class="rounded overflow-hidden flex items-center justify-center border border-dashed border-gray-300 mx-auto"
                  :style="{
                ...
                >
                  <img 
                    v-if="form.data.backgroundImage" 
                    :src="convertLocalPathToFileUrl(form.data.backgroundImage)" 
                    alt="Preview" 
                    class="max-h-full mx-auto my-2"
                    :style="{
                      maxWidth: '100%',
                      width: 'auto',
                      height: 'auto',
...
function convertLocalPathToFileUrl(path: string): string {
  ...
  if (!url.startsWith('file://')) {
    // 如果路径不是以file://开头，并且是绝对路径，添加file://前缀
    if (/^[a-zA-Z]:\//.test(url)) {
      url = `file://${url}`;
    } else if (/^\//.test(url)) {
      // Linux/macOS绝对路径
...
function checkBackgroundImageUrl() {
  if (form.data.backgroundImage) {
    ...
    if (isRemoteUrl) {
      form.data.saveBackgroundImageOption = 1;
    }
  }
}
...
function selectBackgroundImage() {
  ...
  if (filePath) {
    form.data.backgroundImage = filePath;
  }
}
// 确认
async function confirm() {
  ...
  if (!id) {
    ...
    let updatedItem = window.item.update(item);
    if (updatedItem) {
  ...
  let item = window.item.add(
    newItem({
      ...
      data: newItemData({
        ...
        backgroundImage: form.data.backgroundImage,
        saveBackgroundImageOption: form.data.saveBackgroundImageOption,
        localBackgroundImage: form.data.localBackgroundImage,
        projectBackgroundImage: form.data.projectBackgroundImage,
      }),
