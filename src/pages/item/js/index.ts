...
function setItemWidth() {
  ...
  for (let i = 0; i < itemListElementList.length; i++) {
    ...
    // 间距
    let gap = 16; // 统一使用16px间距
    
    // 获取项目
    let items = itemListElement.querySelectorAll(".item");
    
    if (layout === "cover") {
      // 封面布局：类似应用商店的自适应卡片
      const coverCardMinWidth = 200; // 封面卡片最小宽度
      const coverCardMaxWidth = 300; // 封面卡片最大宽度
      
      // 计算每行可容纳的卡片数量
      let numPerRow = Math.floor((width + gap) / (coverCardMinWidth + gap));
      numPerRow = Math.max(1, numPerRow); // 至少显示1个
      
      // 计算卡片宽度，考虑间距
      let cardWidth = (width - (numPerRow - 1) * gap) / numPerRow;
      
      // 限制卡片宽度在合理范围内
      cardWidth = Math.max(coverCardMinWidth, Math.min(cardWidth, coverCardMaxWidth));
      
      // 重新计算每行数量，确保卡片宽度合适
      numPerRow = Math.max(1, Math.floor((width + gap) / (cardWidth + gap)));
      
      // 最终宽度计算
      cardWidth = (width - (numPerRow - 1) * gap) / numPerRow;
      ...
      num = Math.max(1, num); // 至少显示1个
      
      // 计算项目宽度
      let itemWidth = (width - (num - 1) * gap) / num;
