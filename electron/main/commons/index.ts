...
function getURLInfo(windowName: string, url: string, redirect: boolean) {
  ...
  try {
    // 剥离URL中的片段标识符（#后面的部分），因为HTTP请求中不会发送这部分
        ...
        function (error, response, body) {
          ...
          if (
            ...
            if (content && content.trim() !== "" && redirect) {
              ...
              // 解析HTML并返回信息，传递处理后的URL用于图标处理
              analysisHTML(windowName, processedUrl, body);
            }
          } else {
            // 输出错误信息以便调试
            console.log("获取URL信息失败:", { 
              error: error?.message, 
...
function analysisHTML(windowName: string, url: string, data: string) {
  ...
  try {
    ...
    if (iconTags.length === 0) {
      ...
      iconTags.push({
        attr: (name: string) => {
          if (name === "href") return "/favicon.ico";
          return undefined;
        }
      });
    }
    ...
    // 处理图标URL，确保完整
    const processIconUrl = (href: string | undefined, baseUrl: string): string => {
      if (!href || href.trim() === "") return "";
      
      let processedUrl = href.trim();
    ...
    // 尝试下载图标的函数
    const tryDownloadIcon = (index: number) => {
      if (index >= iconTags.length) {
        // 所有图标都尝试过了，仍未成功
        if (result.name && result.name.trim() !== "") {
          result.status = true;
        }
      ...
      let iconTag = iconTags[index];
      let href = iconTag.attr("href");
      let iconURL = processIconUrl(href, url);
      
      if (!iconURL) {
        // 当前图标URL无效，尝试下一个
        tryDownloadIcon(index + 1);
        return;
      }
      
      // 重试
      const operation = retry.operation({
        retries: 3, // 每个图标最多重试 3 次
        factor: 1, // 每次重试之间的时间间隔加倍
        minTimeout: 1000, // 第一次重试之前等待的时间
        maxTimeout: 5000, // 最长等待时间
      });
