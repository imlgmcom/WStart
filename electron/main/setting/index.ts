...
function setShortcutKey(setting: Setting = global.setting) {
  // 保存当前主窗口是否可见
  const isMainWindowVisible = mainWindowExist() && global.mainWindow.isVisible();
  // 暂时移除主窗口的 blur 事件监听器，避免在设置快捷键过程中窗口失去焦点导致最小化
  if (isMainWindowVisible && global.setting.general.hideLoseFocus) {
    global.mainWindow.removeAllListeners("blur");
  }
