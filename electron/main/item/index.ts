...
function convertTarget(idList: Array<number>, type: "Absolute" | "Relative") {
  ...
  for (let item of itemList) {
    ...
    if (
      ...
      // 转换本地背景图路径
      if (item.data.localBackgroundImage && isAbsolutePath(item.data.localBackgroundImage)) {
        item.data.localBackgroundImage = convertPath(item.data.localBackgroundImage);
      }
